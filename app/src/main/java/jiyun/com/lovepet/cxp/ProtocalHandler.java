...
public abstract class ProtocalHandler {
	...
	protected String getRequestString(String userdata) {
		...
		sb.append("loctype=1");
		sb.append("&tableid=").append(AppUtils.TABLE_ID);
		sb.append("&key=").append(AppUtils.KEY);
		if (userdata.indexOf("{") == -1) {
			sb.append("&ids=").append(userdata);
		} else {
	...
	protected String getUrl(String str) {
		...
		if (str.equals("create")) {
			url = AppUtils.SAVE_URL;
		} else if (str.equals("update")) {
			url = AppUtils.UPDATE_URL;
		} else if (str.equals("delete")) {
			url = AppUtils.DELETE_URL;
		}
		return url;
	}
	...
	public String getData(String str) {
		String result = null;
		result = getDataMayThrow(str);
		return result;
	}

	private String getDataMayThrow(String str) {
		HttpURLConnection conn = null;
		InputStream input = null;
		String result = null;
		recommandURL = getUrl(str);
