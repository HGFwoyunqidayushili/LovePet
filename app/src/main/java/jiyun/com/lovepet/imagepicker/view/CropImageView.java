package jiyun.com.lovepet.imagepicker.view;
...
/**
 * ================================================ 作 者：廖子尧 版 本：1.0
 * 创建日期：2016/1/7 描 述： Matrix 的9个值分别为 缩放 平移 倾斜 MSCALE_X MSKEW_X MTRANS_X MSKEW_Y
 * MSCALE_Y MTRANS_Y MPERSP_0 MPERSP_1 MPERSP_2 修订历史：
 * ================================================
 */
...
public class CropImageView extends ImageView {
	...
	public CropImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		mFocusWidth = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, mFocusWidth, getResources()
						.getDisplayMetrics());
		mFocusHeight = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, mFocusHeight, getResources()
						.getDisplayMetrics());
		...
		mMaskColor = a.getColor(R.styleable.CropImageView_cropMaskColor,
				mMaskColor);
		mBorderColor = a.getColor(R.styleable.CropImageView_cropBorderColor,
				mBorderColor);
		mBorderWidth = a.getDimensionPixelSize(
				R.styleable.CropImageView_cropBorderWidth, mBorderWidth);
		mFocusWidth = a.getDimensionPixelSize(
				R.styleable.CropImageView_cropFocusWidth, mFocusWidth);
		mFocusHeight = a.getDimensionPixelSize(
				R.styleable.CropImageView_cropFocusHeight, mFocusHeight);
		mDefaultStyleIndex = a.getInteger(R.styleable.CropImageView_cropStyle,
				mDefaultStyleIndex);
	...
	private void initImage() {
		...
		float fitFocusScale = getScale(mImageWidth, mImageHeight, mFocusWidth,
				mFocusHeight, true);
		mMaxScale = fitFocusScale * MAX_SCALE;
		// 适配显示图片的ImageView的缩放比例（图片至少有一边是铺满屏幕的显示的情形）
		float fitViewScale = getScale(mImageWidth, mImageHeight, viewWidth,
				viewHeight, false);
		...
		float scale = fitViewScale > fitFocusScale ? fitViewScale
				: fitFocusScale;
		// 图像中点为中心进行缩放
		...
		float transX = mFocusMidPoint.x
				- (mImageMatrixValues[2] + mImageWidth * mImageMatrixValues[0]
						/ 2); // X轴方向的位移
		float transY = mFocusMidPoint.y
				- (mImageMatrixValues[5] + mImageHeight * mImageMatrixValues[4]
						/ 2); // Y轴方向的位移
	...
	private float getScale(int bitmapWidth, int bitmapHeight, int minWidth,
			int minHeight, boolean isMinScale) {
		float scale;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (Style.RECTANGLE == mStyle) {
			...
			mFocusPath.addCircle(mFocusMidPoint.x, mFocusMidPoint.y, radius,
					Path.Direction.CCW);
			canvas.save();
			canvas.clipRect(0, 0, getWidth(), getHeight());
			canvas.clipPath(mFocusPath, Region.Op.DIFFERENCE);
		...
		case MotionEvent.ACTION_POINTER_UP:
			if (mode == DRAG) {
				if (spacing(pA, pB) < 50) {
					...
					if (now - doubleClickTime < 500
							&& spacing(pA, doubleClickPos) < 50) {
				...
				int rotateLevel = (int) Math.floor((rotation + Math.PI / 4)
						/ (Math.PI / 2));
				if (rotateLevel == 4)
					rotateLevel = 0;
				matrix.set(savedMatrix);
	...
	private void fixScale() {
		...
		float currentScale = Math.abs(imageMatrixValues[0])
				+ Math.abs(imageMatrixValues[1]);
		float minScale = getScale(mRotatedImageWidth, mRotatedImageHeight,
				mFocusWidth, mFocusHeight, true);
	...
	private float maxPostScale() {
		float imageMatrixValues[] = new float[9];
		matrix.getValues(imageMatrixValues);
		float curScale = Math.abs(imageMatrixValues[0])
				+ Math.abs(imageMatrixValues[1]);
	...
	public Bitmap getCropBitmap(int expectWidth, int exceptHeight,
		...
		return makeCropBitmap(srcBitmap, mFocusRect, getImageMatrixRect(),
	...
	private RectF getImageMatrixRect() {
		...
		rectF.set(0, 0, getDrawable().getIntrinsicWidth(), getDrawable()
				.getIntrinsicHeight());
	 ...
	 *            当前图片在屏幕上的显示矩形区域
	 * @param expectWidth
	 *            希望获得的图片宽度，如果图片宽度不足时，拉伸图片
	 * @param exceptHeight
	 *            希望获得的图片高度，如果图片高度不足时，拉伸图片
	 * @param isSaveRectangle
	 *            是否希望按矩形区域保存图片
	 * @return 裁剪后的图片的Bitmap
	 */
	private Bitmap makeCropBitmap(Bitmap bitmap, RectF focusRect,
			RectF imageMatrixRect, int expectWidth, int exceptHeight,
			boolean isSaveRectangle) {
		...
		try {
			...
			if (expectWidth != width || exceptHeight != height) {
				bitmap = Bitmap.createScaledBitmap(bitmap, expectWidth,
						exceptHeight, true);
				if (mStyle == Style.CIRCLE && !isSaveRectangle) {
					// 如果是圆形，就将图片裁剪成圆的
					int length = Math.min(expectWidth, exceptHeight);
					int radius = length / 2;
					Bitmap circleBitmap = Bitmap.createBitmap(length, length,
					...
					canvas.drawCircle(expectWidth / 2f, exceptHeight / 2f,
							radius, paint);
					bitmap = circleBitmap;
				}
			}
	 ...
	 *            是否希望按矩形区域保存图片
	 */
	public void saveBitmapToFile(File folder, int expectWidth,
			int exceptHeight, boolean isSaveRectangle) {
		if (mSaving)
			return;
		mSaving = true;
	...
	private File createFile(File folder, String prefix, String suffix) {
		if (!folder.exists() || !folder.isDirectory())
			folder.mkdirs();
		try {
			File nomedia = new File(folder, ".nomedia"); // 在当前文件夹底下创建一个
															// .nomedia 文件
			if (!nomedia.exists())
	...
	private void saveOutput(Bitmap croppedImage,
			Bitmap.CompressFormat outputFormat, File saveFile) {
	...
	public void setOnBitmapSaveCompleteListener(
			OnBitmapSaveCompleteListener listener) {
		mListener = listener;
	}
